...
func (s *InboundService) addClientTraffic(tx *gorm.DB, traffics []*xray.ClientTraffic) (err error) {
	...
	// Load traffic ratios for all clients in one query
	clientEmails := make([]string, 0, len(dbClientTraffics))
	for _, ct := range dbClientTraffics {
		clientEmails = append(clientEmails, ct.Email)
	}
	type ratioRow struct {
		Email        string
		TrafficRatio float64
	}
	var ratioRows []ratioRow
	tx.Model(&model.ClientRecord{}).
		Select("email, traffic_ratio").
		Where("email IN ?", clientEmails).
		Scan(&ratioRows)
	ratioByEmail := make(map[string]float64, len(ratioRows))
	for _, r := range ratioRows {
		ratioByEmail[r.Email] = r.TrafficRatio
	}
	...
	for dbTraffic_index := range dbClientTraffics {
		...
		ratio := ratioByEmail[dbClientTraffics[dbTraffic_index].Email]
		if ratio <= 0 {
			ratio = 1.0
		}
		dbClientTraffics[dbTraffic_index].Up += int64(float64(t.Up) * ratio)
		dbClientTraffics[dbTraffic_index].Down += int64(float64(t.Down) * ratio)
		if t.Up+t.Down > 0 {
